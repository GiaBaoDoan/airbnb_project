  ...
  return (
    <CssContainer>
      <section className="py-12">
        ...
        <div className="grid grid-cols-4 max-sm:grid-cols-1 max-xl:grid-cols-3 max-lg:grid-cols-2  max-md:grid-cols-2  max-sm:gap-3 gap-5 mt-5">
          {location?.map((i) => {
            return (
              <div
                onClick={() => navigate(`/results/${i.id}`)}
                key={i.id}
                className="flex p-3 items-center space-x-3 location cursor-pointer hover:bg-black/5 transition-all  rounded-lg"
              >
                <img
                  src={i.hinhAnh}
                  className="w-16 h-16 object-cover rounded-lg"
                />
                <div className="space-y-6 flex-1 text-lg max-sm:text-sm">
                  <p>{i.tenViTri}</p>
