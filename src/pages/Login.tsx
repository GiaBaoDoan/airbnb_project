...
import { setIsremember } from "store/quanLyNguoiDung/LoginSlice";
import { getThongTinUserThunk } from "store/CapNhatThongTin/thunk";
import { useEffect, useState } from "react";
const Register = () => {
  const { isLoading } = useSelector((state: RootState) => state.LoginReducer);
  ...
  const onSubmit = async (data) => {
    await dispatch(loginThunk(data))
      .unwrap()
      .then(async (res) => {
        localStorage.setItem("id", String(res?.user.id));
        ...
        await dispatch(getThongTinUserThunk(String(res?.user.id)));
        navigate(`/`);
      })
  ...
  return (
    <RegisterCSS>
      ...
      <div className="box relative singin max-ipad:!w-[90%]   ">
        <form
          noValidate
          onSubmit={handleSubmit(onSubmit)}
          className="Form p-[10px] max-ipad:!w-full max-tablet:!h-full overflow-auto  bg-white "
        >
          <div className="w-full flex max-ip678Plus:flex-col ">
            <div className="m-[25px] max-ip678Plus:m-[10px]">
              <svg
                onClick={() => navigate("/")}
                width="102"
            ...
            <h2 className="text-[35px] max-tablet:text-[25px] mt-[20px] max-ip678Plus:mt-[8px] flex w-full justify-center  text-[#00308F] font-600">
              Đăng nhập
            </h2>
          </div>
          <div className="p-[20px] max-ip678Plus:px-[20px] max-ip678Plus:py-[5px]">
            <div className="mb-[14px] grid grid-cols-1">
              <div className="flex items-center ">
                <label className="block mr-[12px] mb-0 text-sm outline-none font-medium text-gray-900 dark:text-white">
                  {/* <FontAwesomeIcon icon={icons[1]} className="text-[20px]" /> */}
                </label>
                <Input
              ...
              <div className="mt-[14px]">
                <div className="flex items-center ">
                  <label className="block  mr-[12px] text-sm font-medium text-gray-900 dark:text-white">
                    {/* <FontAwesomeIcon icon={icons[2]} className="text-[20px]" /> */}
                  </label>
                  <Input
                    name="password"
          ...
          <div className="p-[20px] max-ip678Plus:px-[20px] max-ip678Plus:py-[5px]">
            <div className="flex justify-between max-ip678Plus:justify-center items-center">
              <div className="max-ip678Plus:hidden">
                <NavLink
                  to={"/login"}
                  className={"text-[#FF4500] text-[20px] hidden"}
                  style={{ display: "flex", alignItems: "center" }}
                >
                  {" "}
                  <span
                    onClick={() => setColor(!color)}
                    className="check-icon mr-[5px] max-tablet:hi"
                    style={{
                  ...
                  >
                    {/* <FontAwesomeIcon
                      icon={icons[0]}
                      className="text-[16px]"
                      style={{ color: `${color ? "white" : "#FF4500"}` }}
                    /> */}
                  </span>
              ...
              <Button
                ...
                className="btn-submit max-ip678Plus:!w-full max-ip678Plus:ml-[30px]"
                // className="bg-red-600  rounded-[40px] sm:w-[400px] lg:w-[300px] text-[20px] h-[50px] "
              >
                {isLoading ? (
                  <span>
                    {/* <FontAwesomeIcon
                      icon={icons[3]}
                      className="text-[20px] text-[white] rotate-icon rotating"
                    /> */}
                  </span>
                ) : (
                  <span
                    className="mr-[12px]"
                    onClick={() => dispatch(setIsremember(true))}
                  >
                    Đăng nhập
                  </span>
                )}
            ...
            <div className="my-[20px] text-center">
              <span className="text-[18px]">Chưa có tài khoản?</span>
              <NavLink
                to={PATH.Register}
                className={"text-[#FF4500] text-[20px] "}
              >
                {" "}
                Đăng kí ngay
              </NavLink>
...
const RegisterCSS = styled.div`
  ...
  .rotate-icon {
    transition: transform 1s ease-in-out; /* Định nghĩa hiệu ứng chuyển động */
  }
  .rotating {
    animation: rotate 2s linear infinite; /* Áp dụng hiệu ứng xoay */
  }
  .wrapper {
    /* position: fixed;
    top: 50%;
  ...
  @keyframes rotate {
    0% {
      transform: rotate(0deg); /* Góc ban đầu */
    }
    100% {
      transform: rotate(360deg); /* Góc xoay đầy đủ */
    }
  }
`;
